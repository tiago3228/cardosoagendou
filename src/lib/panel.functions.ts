...
export const getAgenda = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const appointments = await context.supabase
      .from("appointments")
      .select(
        "id, starts_at, ends_at, status, presence_status, client_name, client_whatsapp, total_price_cents, duration_minutes, notes, professional_id, professionals:professional_id (name), appointment_services (service_name, price_cents, duration_minutes)",
      )
