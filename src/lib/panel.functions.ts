  ...
  .handler(async ({ context }) => {
    ...
    const [business, subscription, professionals, profile] = await Promise.all([
      context.supabase
        .from("businesses")
        .select(
          "id, slug, name, business_type, description, logo_url, cover_url, whatsapp, email, address, booking_policy, slot_interval_minutes, min_notice_minutes, max_advance_days, timezone",
        )
        .eq("id", businessId)
        .maybeSingle(),
      context.supabase
