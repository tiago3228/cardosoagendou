  ...
  .handler(async ({ context }) => {
    ...
    const [business, subscription, professionals, profile] = await Promise.all([
      context.supabase
        .from("businesses")
        .select(
          "id, slug, name, business_type, description, description_font, description_color, logo_url, cover_url, whatsapp, email, address, instagram_url, google_review_url, show_address, show_instagram, show_whatsapp, booking_policy, slot_interval_minutes, min_notice_minutes, max_advance_days, cancellation_deadline_hours, primary_color, secondary_color, timezone",
        )
        .eq("id", businessId)
        .maybeSingle(),
      context.supabase
...
export const getAgenda = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const appointments = await context.supabase
      .from("appointments")
      .select(
        "id, starts_at, ends_at, status, client_name, client_whatsapp, total_price_cents, duration_minutes, notes, cancel_reason, blocks_agenda, professional_id, professionals:professional_id (name), appointment_services (service_name, price_cents, duration_minutes)",
      )
