  ...
  .handler(async ({ context }) => {
    ...
    const [business, subscription, professionals, profile] = await Promise.all([
      context.supabase
        .from("businesses")
        .select(
          "id, slug, name, business_type, description, logo_url, cover_url, whatsapp, email, address, instagram_url, show_address, show_instagram, show_whatsapp, booking_policy, slot_interval_minutes, min_notice_minutes, max_advance_days, cancellation_deadline_hours, primary_color, secondary_color, timezone",
        )
        .eq("id", businessId)
        .maybeSingle(),
      context.supabase
