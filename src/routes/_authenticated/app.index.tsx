...
function AgendaPage() {
  ...
  const whatsappShareUrl = bookingUrl
    ? `https://wa.me/?text=${encodeURIComponent(`Olá! Agende seu horário online com ${panel.business?.name ?? "a nossa equipe"}: ${bookingUrl}`)}`
    : "";
  ...
  return (
    <div className="space-y-5">
      ...
      {bookingUrl ? (
        <section className="rounded-lg border border-border bg-primary/5 px-4 py-4 md:px-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase text-primary">Agendamento online</p>
              <p className="mt-1 font-medium text-foreground">
                Envie este link para seus clientes agendarem sozinhos
              </p>
              <p className="mt-1 truncate text-sm text-muted-foreground">{bookingUrl}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <Button size="sm" variant="outline" asChild disabled={!whatsappShareUrl}>
      ...
      <section className="rounded-lg border border-border bg-card p-3 md:p-4">
        ...
        {!agenda.isLoading && visible.length === 0 ? (
          <div className="m-1 mt-4 rounded-lg border border-dashed border-border p-10 text-center">
            ...
            <p className="mt-3 text-sm font-medium">Nenhum agendamento neste período</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Clique em Agendar para criar o primeiro atendimento — o sistema validará os horários
              automaticamente.
            </p>
          </div>
        ) : null}
